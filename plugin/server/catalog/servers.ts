...
import { type Kit, type McpServers, type ProxySpec, PASEO_SERVER, TEAM_SERVER, can, paseoToolsPolicy, teamServer, toolsOf } from "./kit.ts";
...
export function choicesFor(kit: Kit, team: Team, roleName: string): Record<string, Record<string, string[]>> {
  ...
  const all = { add_tasks: { role: writers, skills: [...new Set(skills)].sort() }, start_review: { role: holding("review") }, open_lane: { role: holding("lead") }, note: { kind: pages } };
