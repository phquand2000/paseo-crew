...
async function changedBy(cwd: string, tip: string, files: string[]): Promise<string[]> {
  ...
  const run = await git(cwd, ["log", "-z", "--first-parent", "--merges", "--diff-merges=first-parent", "--name-only", "--format=%x01%s", `HEAD..${tip}`]);
  const ids = run.stdout.split("\x01").flatMap((record) => {
    const [subject = "", ...changed] = record.split("\0").map((part) => part.trim()).filter(Boolean);
...
export async function bringLaneIn(task: Task & { worktree: string; branch: string }, lane: Lane): Promise<Synced> {
  ...
  if (copy !== "clean") return { not: copy === "dirty" ? "its copy has work uncommitted" : "git could not read its copy" };
