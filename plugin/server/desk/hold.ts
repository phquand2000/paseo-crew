...
/** Puts a lane on hold for `by`: each seat still working in it gets HOLD past the outbox, and a landing it waited on is called off. */
export async function putOnHold(desk: DeskServices, project: Project, laneId: string, by: string, reason: string): Promise<{ lane: Lane; stopped: string[]; calledOff: boolean } | string> {
