...
import type { AgentRef, Ledger } from "../../domain/ledger.ts";
...
/** Whether a seat is gone: no longer one Paseo lists, so nothing of its own is pending on it. */
type Gone = (agentId: string) => boolean;
...
function carriedOn(ledger: Ledger): Set<string> {
  ...
  const text = [
    ...
    ...Object.values(ledger.tasks)
      .filter((task) => open.has(task.lane))
      .flatMap((task) => [task.title, task.goal, task.context ?? "", ...task.acceptance]),
    ...Object.values(ledger.asks)
...
function stillPending(ledger: Ledger, lane: Lane, of: Entries, carried: Set<string>, gone: Gone, now: number): boolean {
  ...
  const seats = new Set(
    [lane.lead, ...of.tasks.map((task) => task.peer), ...of.agents.map((agent) => agent.id)].filter(
  ...
  return (
    ...
    Object.values(ledger.slots).some(
      (slot) => slot.lane === lane.id || (slot.task !== undefined && taskIds.has(slot.task)) || slot.id === lane.slot,
    ) ||
 ...
 */
export function takeFinished(ledger: Ledger, gone: Gone, now = Date.now()): Taken | undefined {
  const taken: Taken = { lanes: takeLanes(ledger, gone, now), agents: [], asks: [], questions: [] };
  takeUnbound(ledger, gone, now, taken);
...
function takeLanes(ledger: Ledger, gone: Gone, now: number): LaneArchive[] {
  ...
  for (const lane of closed) {
    ...
    taken.push({ lane, tasks, asks, questions, agents, records: {} });
  }
  return taken;
}
...
function takeUnbound(ledger: Ledger, gone: Gone, now: number, taken: Taken): void {
  ...
  for (const agent of Object.values(ledger.agents)) {
    ...
    if (
      Object.values(ledger.asks).some((ask) => ask.status === "open" && (ask.from === agent.id || ask.to === agent.id))
    )
      continue;
  ...
  for (const question of Object.values(ledger.questions)) {
    if (
      ...
      question.openedAt > now - DAY_MS ||
      ledger.agents[question.from] ||
      !gone(question.from)
    )
      continue;
...
export function keepArchived(state: string, taken: Taken): void {
  for (const entry of taken.lanes)
    fileLane(state, entry.lane!.id, (archive) =>
  ...
  const roll = {
    dir: archiveDir(state),
    current: "desk.log",
    prefix: "desk.",
    ext: ".log",
    rotateAt: DESK_ROTATE_BYTES,
    keepBytes: DESK_KEEP_BYTES,
    plain: 1,
  };
  ...
  for (const lane of new Set(loose.map((record) => record.lane))) {
    ...
    fileLane(state, lane, (archive) => {
      for (const record of records) {
        ...
        const cut =
          statSync(file).size > RECORD_TAIL_BYTES
            ? `[the start of this log was cut; its last ${RECORD_TAIL_BYTES} bytes follow]\n`
            : "";
...
export function archiveFinished(services: DeskServices, project: Project, gone: Gone): void {
  ...
  if (taken || filed.length > 0) {
    recordEvent(project, {
      ...
      lanes: taken?.lanes.map((entry) => entry.lane!.id) ?? [],
      agents: taken?.agents.length ?? 0,
      asks: taken?.asks.length ?? 0,
      records: filed.length,
    });
  }
}
