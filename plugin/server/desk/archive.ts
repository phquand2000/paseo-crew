...
/** Lane ids the open work names, a hand-back's file name included; naming one too many only keeps it longer. */
function carriedOn(ledger: Ledger): Set<string> {
  ...
  const text = [
    ...Object.values(ledger.lanes).filter((lane) => open.has(lane.id)).flatMap((lane) => [lane.title, lane.outcome, ...lane.acceptance]),
  ...
  return new Set(text.match(/\bL\d+\b/g) ?? []);
}
