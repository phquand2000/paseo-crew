...
async function readAhead(desk: DeskServices, project: Project, lane: Lane): Promise<{ asks: string[]; facts: string[]; changes: boolean }> {
  ...
  return { asks: askFirstHits(project, change), facts: await landFacts(desk.kit, project, ledger, lane, change), changes: changesStanding(ledger, lane) };
}
...
export const report = defineTool({
  ...
  async handle(desk, caller, args) {
    ...
    const gate = args.ready === true ? await laneGate(desk, caller.project, lane) : undefined;
    // Recorded on the lane the caller still leads: it may have closed, or had its Lead replaced, while the gate ran.
    const still = ledgers.transact(caller.project, (current) => {
    ...
    const posted = await mail.post(to, letter);
    recordEvent(caller.project, { kind: "lane.report", lane: lane.id, ready: args.ready === true, gate: gate?.ok, to: to ?? null, text: posted === "nobody" ? letter.text : undefined });
