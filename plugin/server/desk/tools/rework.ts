...
export const rework = defineTool({
  ...
  async handle(desk, caller, args) {
    ...
    // A task sent back after its merge is read from where the lane stands now, so a cut takes only what comes after.
    const from = asked.task.status === "merged" && asked.task.mode !== "parallel" ? await headSha(asked.lane.worktree!) : undefined;
