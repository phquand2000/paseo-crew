...
import { type Urgency, WATCH_RULES, judge, keyOf, pagesLeft, refund, reserve, told } from "../watching.ts";
...
export const raise: Tool = async ({ ctx, roster }, caller, args) => {
  ...
  // Everything the seat did since the Watcher last raised on it, not only its latest turn: a busy
  // Watcher reads several endings of one seat from one mailbox, and a slot the next ending overwrote
  // handed a finding about the first turn the evidence of the last.
  const filed = ctx.hasReading(project, where);
  const recorded = ctx.takeReading(project, where);
  ...
  //
  // One strike per label per ending. Judged once per finding, three quotes of one fault in one ending
  // were three sightings, so a fault seen once paged on its first appearance under a letter calling it
  // the third time. Each quote is still logged.
  const byLabel = findings.filter((finding, index) => findings.findIndex((other) => other.label === finding.label) === index);
  const judged = await ctx.watching(project, (current) => {
    let next = current;
    let worst: Urgency = "log";
    const paged: Finding[] = [];
    const counts = new Map<string, number>();
    for (const finding of byLabel) {
    ...
    for (const finding of findings) ctx.event(project, { kind: "watch", agent: caller.id, label: finding.label, where, quote: finding.quote, urgency: paged.some((entry) => entry.label === finding.label) ? "page" : worst === "log" ? "log" : "digest", count: counts.get(finding.label) ?? 0 });
    // The rationing and the reservation happen here, inside the same lock that read the budget.
    const room = pagesLeft(next, now, rules);
    ...
    return { save: reserve(next, sending.length, now), result: { worst, paged, counts, sending } };
  });
  const { worst, paged, counts, sending } = judged;
  ...
  // is never held back.
  const waiting = paged.filter((finding) => !sending.includes(finding));
  ...
  if (!to) {
    ...
    // would end this turn over something the Watcher did right. The pages it reserved come back.
    await ctx.watching(project, (current) => ({ save: refund(current, sending.length, now), result: undefined }));
  ...
  // The pages were reserved when the budget was read; what the letters settle is recorded now.
  const charged = sending.map((finding) => keyOf(where, finding.label));
  await ctx.watching(project, (current) => ({ save: told(current, charged), result: undefined }));
