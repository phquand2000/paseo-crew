...
export const setProject = defineTool({
  name: "set_project",
  input: z.strictObject({ base: z.string().optional(), gate: z.string().optional(), gateTimeoutMinutes: z.number().optional(), gateOn: z.enum(["lane", "task"]).optional(), serialOnly: z.array(z.string()).optional(), landAs: z.enum(["squash", "merge", "ff"]).optional(), laneHome: z.enum(LANE_HOMES).optional(), askFirst: z.array(z.string()).optional(), riskRules: z.array(RiskRule).optional() }),
  async handle(_desk, caller, args) {
    ...
    return ok(`Base ${next.base ?? "unset"}; gate ${next.gate || "none"}, run per ${next.gateOn}; gate timeout ${next.gateTimeoutMinutes} minutes; lanes land as ${next.landAs}; ${home}; ${asked}; ${rules}.`);
  },
});
