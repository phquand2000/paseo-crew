...
function askNext(ask: Ask): string {
  if (ask.task) return `Its Lead is gone: answer ${ask.id} if you can; replace_lead puts a new Lead on the lane where it stands.`;
  if (ask.kind === "question") return `If CONTEXT.md settles it, answer ${ask.id}; else ask the Human with your recommendation, write their answer into CONTEXT.md, then answer. The Lead runs on its default meanwhile.`;
...
function answeredNext(question: Question): string {
  ...
  if (question.class === "irreversible") return "Carry their choice into the lane, and write it into CONTEXT.md if it settles the concept";
  if (question.answer?.choice !== question.recommend) return "Turn round what went ahead on your recommendation, and write their choice into CONTEXT.md if it settles the concept";
...
export const askLetters = {
  askTo(ask: Ask, from: string, reader: "lead" | "supervisor"): Letter {
    const next = reader === "lead" ? `Answer ${ask.id} from the brief and the code; if only the owner can, ask up and tell the Peer to wait.` : askNext(ask);
    return mail("ask", [ask.id], [`ASK ${ask.id} (${ask.kind}) from ${from}`, "", ask.text, ...theirDefault(ask)].join("\n"), next);
  },
  ...
  answeredFor(ask: Ask, by: string, leads = true): Letter {
    ...
    return mail("answeredFor", [ask.id], text, leads ? "If this changes what you were going to do, say so in your next report." : "If it changes a decision of yours, carry that into the lane.");
  },

  reminder(ask: Ask, minutes: number): Letter {
    return mail("remind", [ask.id, ask.reminders], `STILL OPEN after ${minutes} minutes: ask ${ask.id} (${ask.kind}): ${firstLine(ask.text)}`, "Answer it now: whoever asked is waiting on you.");
  },

  escalated(ask: Ask, minutes: number, lane: string): Letter {
    const text = [`UNANSWERED ${ask.id} in ${lane}: a Peer has waited ${minutes} minutes on its Lead.`, "", ask.text, ...theirDefault(ask)].join("\n");
    return mail("escalate", [ask.id], text, "Take the smallest step that unblocks the Peer, often answering it yourself (its Lead is told); if the Lead looks stuck, read its record first.");
  },
};
