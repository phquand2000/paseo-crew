 ...
 */
export async function stowCopy(desk: DeskServices, project: Project, lane: Lane, retired: Task[], how: Stowing): Promise<{ kept: string[]; note: string }> {
  ...
  const off = holder ? { branch: holder.branch!, dropBranch: holder.branch, into: lane.branch } : { branch: lane.branch };
  const kept: string[] = [];
  ...
  if (lane.onBranch && holder) await desk.slots.putAway({ project, restore: lane.branch, lane: lane.id, ...off }, how.writers);
  ...
  if (!lane.onBranch && (!lane.slot || !how.kept)) {
    ...
    const branch = await desk.slots.putAway({ project, slot: lane.slot, restore: lane.base, lane: lane.id, ...off, ...drop }, how.writers);
    if (branch) kept.push(branch);
  }
  const now = lane.worktree && (lane.slot ? how.kept : how.writers.length === 0) ? await currentBranch(lane.worktree) : undefined;
...
function copyNote(lane: Lane, { kept, writers }: Stowing, held: boolean, stuck?: string): string {
  ...
  if (lane.slot && kept) return `Its working copy ${lane.slot} stays with its Lead${stuck ? `, still on ${stuck}` : ""}.`;
  const to = lane.onBranch ? lane.branch : lane.base;
  if (stuck) return `The project's own copy is still on ${stuck}: git would not take it to ${to} as it stands, and each round tries again.`;
  const where = lane.slot ? "Its working copy is put away" : `The project's own copy goes back to ${to}`;
  return writers.length > 0 ? `${where} once ${writers.join(" and ")} finish the turn they are in.` : lane.slot ? "Its working copy is put away." : `The project's own copy is back on ${to}.`;
}
