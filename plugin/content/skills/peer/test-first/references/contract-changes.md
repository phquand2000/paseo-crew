...
1. **Update every consumer in the same commit.** Find every shipping producer, consumer and generated artifact with the contract search on the old name, regenerate generated files with the repository's generator, and send a `DEPENDENCY_REQUEST` for any outside your owned scope. One left on the old shape breaks at runtime.
