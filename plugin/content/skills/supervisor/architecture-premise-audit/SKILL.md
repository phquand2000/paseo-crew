...
4. **Compare every slice.** What demonstrated requirement forces each mechanism? Does cost follow useful work? Are the normal and exceptional paths reversed? What is lost by removing or moving it? Use `~/.local/share/seatworks-v2/guides/STRUCTURAL_LENSES.md` as search lenses; its domain examples show the detail an expected mechanism needs.
