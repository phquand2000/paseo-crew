...
High-risk work, per `FEATURE_INTAKE.md`, gets `docs/exec-plans/active/SLUG.md`;
...
Current state only: an update replaces its row or line. Guide: PLANS.md
...
| A ruling that settles a boundary or is hard to reverse | an ADR, per `ADR.md` | one line under Decisions |
| A ruling inside one slice | a `DECISION:` line and the review record | nothing |
| A Reviewer's findings and your ruling on each | a review record, per `REVIEW.md` | the Review cell; carried findings under Acceptance and recovery |
