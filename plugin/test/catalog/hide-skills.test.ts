...
import { hideSkillsSetting } from "../../server/catalog/seat/hide-skills.ts";
...
test("the shipped Codex harness turns off the owner's ~/.agents/skills, each by the path of its SKILL.md", () => {
  ...
  assert.deepEqual(
    hideSkillsSetting(codex, home),
    {
      skills: {
        config: ["tilth", "ultra-review"].map((name) => ({ path: join(own, name, "SKILL.md"), enabled: false })),
      },
    },
...
test("a seat's hidden skills sit beside the kit's own skill settings, not in their place", () => {
  ...
  const files: Record<string, string> = {
    "harness.json": JSON.stringify({
      ...
      skillsDir: "skills",
      hideSkills: { roots: ["HOME/.agents/skills"], setting: "skills.config" },
  ...
  const config = parse(
    readFileSync(join(seatDir(cx, team.roles.lead!.role, cx.harnesses.cx!, home), "config.toml"), "utf-8"),
  ) as { skills: { bundled: { enabled: boolean }; config: unknown } };
