...
import { taskPlacement } from "../../server/desk/opening.ts";

const lane = {
  id: "L1", title: "Lane L1", outcome: "", acceptance: [], outOfScope: [], base: "main", branch: "lane-l1",
  writeSet: [], contracts: [], lead: "seat-lead", opener: "seat-sup", status: "open" as const, openedAt: 0, tasks: 2,
};
...
function heldBy(status: string): string | undefined {
  ...
  ledger.tasks["L1-T1"] = {
    id: "L1-T1", lane: "L1", kind: "code", mode: "lane", title: "Task", goal: "", acceptance: [], hints: [], holds: [],
    outOfScope: [], peer: "seat-peer", status: status as never, openedAt: 0, updatedAt: 0, silent: 0,
  };
...
test("a task for the lane's copy is told why the task there holds it, from where that task stands", () => {
  ...
  assert.match(heldBy("failed") ?? "", /^L1-T1 failed to merge and is waiting on you, and it still holds the lane's working copy/);
