...
import { closeIncidentsOf, notice, retell } from "../../server/desk/watch/notice.ts";
...
test("the same thing seen of one seat, however often and however concurrently, is one incident", async () => {
  ...
  const results = await Promise.all([
    notice(services, project, seat, [stuck]),
    notice(services, project, seat, [stuck]),
    notice(services, project, seat, [{ ...stuck, quote: "again" }]),
  ]);
  ...
  await notice(services, project, seat, [
    { kind: "destructive", level: "page", quote: "rm -rf /", facts: ["destructive"] },
  ]);
  assert.equal(
    Object.keys(loadIncidents(project.state).items).length,
    3,
    "another seat, or another kind, is another incident",
  );
});
...
test("a mark goes on the incident named and closes it, and a sighting of something else opens a new one", async () => {
  ...
  const reply = await markIncident.handle(services, supervisor, {
    id: "I2",
    verdict: "noise",
    note: "expected: a normal retry of `curl -H 'Authorization: Bearer 9f8e7d6c5b4a39281706'`",
  });
  ...
  const reopened = await notice(services, project, { id: "peer-2", provider: "sw2-peer-claude/claude-opus-5" }, [
    { ...stuck, quote: "the same action failing 3 times: npm run build" },
  ]);
  assert.deepEqual(
    reopened.opened.map((incident) => incident.id),
    ["I3"],
  );
...
test("a fact at note level is kept as evidence and opens no incident; a page is ranked first", () => {
  ...
  const ranked = decide([
    { kind: "stuck", level: "attend", quote: "q" },
    { kind: "destructive", level: "page", quote: "rm -rf /" },
  ]);
  assert.deepEqual(
    ranked.map((finding) => finding.kind),
    ["destructive", "stuck"],
  );
});
...
test("what was held because nobody could be told is told on its next sighting once somebody can, and only then goes quiet", async () => {
  ...
  assert.equal(
    loadIncidents(project.state).items.I1!.quote,
...
test("an incident book that cannot be read is refused rather than started again from nothing", async () => {
  ...
  await assert.rejects(
    notice(services, project, { id: "peer-1", provider: "sw2-peer-claude/claude-opus-5" }, [stuck]),
    /could not be read: [\s\S]*Nothing was written over it/,
  );
...
test("what was held for nobody is told by the round once somebody sits down, and a sighting after the letter is kept beside what was told", async () => {
  ...
  const peer = { id: "peer-1", provider: "sw2-peer-claude/claude-opus-5" };
  await notice(services, project, peer, [
    { kind: "destructive", level: "page", quote: "rm -rf src", facts: ["destructive"] },
  ]);
  ...
  await notice(services, project, peer, [
    { kind: "destructive", level: "page", quote: "git push --force origin main", facts: ["destructive"] },
  ]);
...
test("a condition the Supervisor marked noise is counted, not raised again, unless it pages", async () => {
  ...
  const absent = [
    {
      ...
      quote: "npm run build failed 3 times: src/pointer.js does not exist yet",
      facts: ["stuck"],
    },
  ];
  ...
  assert.equal(
    (
      await markIncident.handle(services, supervisor, {
        id: "I1",
        verdict: "noise",
        note: "expected: it is being written in parallel by L1-T1",
      })
    ).ok,
    true,
  );
  ...
  await notice(services, project, peer, [
    { ...absent[0]!, quote: "npm run build failed 3 times: src/patch.js does not exist yet" },
  ]);
  await notice(services, project, peer, [
    { kind: "destructive", level: "page", quote: "rm -rf /tmp/verify-t1", facts: [] },
  ]);
  assert.equal(
    (
      await markIncident.handle(services, supervisor, {
        id: "I3",
        verdict: "noise",
        note: "expected: its own scratch directory",
      })
    ).ok,
    true,
  );
  ...
  assert.deepEqual(
    Object.keys(loadIncidents(project.state).items),
    ["I1", "I2", "I3", "I4"],
...
test("an incident is never addressed to the seat it is about", async () => {
  ...
  const result = await notice(services, project, { id: "peer-1", provider: "sw2-peer-claude/claude-opus-5" }, [
...
test("the list carries what each seat was asked", async () => {
  ...
  ledger.tasks["L1-T1"] = {
    ...
    goal: "show the empty cart message",
    acceptance: ["empty cart renders it"],
    hints: ["src/cart/**"],
    holds: [],
    outOfScope: ["checkout"],
    peer: "peer-1",
  ...
  assert.match(
    listed,
    /- L1-T2 Receipt: goal show the empty cart message; acceptance empty cart renders it; holds src\/receipt\/\*\*; out of scope checkout/,
  );
});
...
test("a kind most of whose last ten marks were noise is held on probation, a page never is, and marks that turn it round let it go again", async () => {
  ...
  const marks = (useful: number, count = 10, unknown = 0) => {
    mkdirSync(project.state, { recursive: true });
    const marked = (kind: string, level: string, n: number) => ({
      id: `I${kind}${n}`,
      seat: `old-${n}`,
      where: "old",
      kind,
      ...
      open: false,
      told: n,
      label: n >= count ? "unknown" : n < useful ? "useful" : "noise",
      closed: 1000 + n,
    });
    ...
    writeFileSync(
      join(project.state, "incidents.json"),
      JSON.stringify({ next: 100, items: Object.fromEntries(items.map((item) => [item.id, item])) }),
    );
  };
  marks(4);
  await notice(services, project, seat(1), [stuck]);
  await notice(services, project, seat(2), [
    { kind: "destructive", level: "page", quote: "rm -rf /", facts: ["destructive"] },
  ]);
  const held = Object.values(loadIncidents(project.state).items).filter((item) => item.open);
  assert.deepEqual(
    held.map((item) => [item.kind, item.held ?? null, item.told !== undefined]),
    [
  ...
  assert.match(
    (await incidents.handle(services, supervisor, {})).text,
  ...
  assert.ok(
    Object.values(loadIncidents(project.state).items).find((item) => item.seat === "peer-3")!.told,
    "half of the last ten useful is not probation, and a mark of unknown says nothing either way",
  );
  marks(0, 9);
  await notice(services, project, seat(4), [stuck]);
  assert.ok(
    Object.values(loadIncidents(project.state).items).find((item) => item.seat === "peer-4")!.told,
    "nine marks judge nothing",
  );
});
