...
test("an answer that is not every question's probability is not an answer", async () => {
  const ask = (body: unknown) =>
    decisionsJudge(spec, "k", endpoint([{ status: 200, body }]).fetcher).ask({}, { a: gap, b: gap });
  ...
  await assert.rejects(
    ask(answered({ type: "choice", choice: "x", confidence: 1 })),
    /the answer to b is not a probability/,
  );
  await assert.rejects(
    ask({ answers: { a: { type: "noul", noul: 0.4 }, b: { type: "noul", noul: 1 } } }),
...
test("a choice is one of the question's own criteria, with how sure the sensor is of it", async () => {
  ...
  assert.deepEqual(
    (await decisionsJudge(spec, "k", endpoint([{ status: 200, body: recorded }]).fetcher).ask({}, { team })).answers,
    { team: { choice: "payments", confidence: 0.75 } },
  );
  const ask = (answer: unknown) =>
    decisionsJudge(spec, "k", endpoint([{ status: 200, body: { answers: { team: answer }, model: "m" } }]).fetcher).ask(
      {},
      { team },
    );
  await assert.rejects(
    ask({ type: "choice", choice: "legal", confidence: 0.9 }),
    /the answer to team is not one of its choices/,
  );
  await assert.rejects(
    ask({ type: "choice", choice: "toString", confidence: 0.9 }),
    /the answer to team is not one of its choices/,
  );
  await assert.rejects(
    ask({ type: "choice", choice: "payments", confidence: 2 }),
    /the answer to team has a confidence outside 0 to 1/,
  );
  assert.deepEqual(
    (await ask({ type: "choice", choice: "payments" })).answers,
...
test("a busy or failing endpoint is asked again as often as the sensor allows, and a refusal retrying cannot fix is not", async () => {
  ...
  assert.deepEqual((await decisionsJudge(spec, "k", busy.fetcher).ask({}, { is_bug: gap })).answers.is_bug, {
    noul: 0.96,
  });
  ...
  await assert.rejects(
    decisionsJudge(spec, "k", refused.fetcher).ask({}, { is_bug: gap }),
...
test("a reply that stalls, before or after its headers, is no answer within the sensor's time", async () => {
  ...
  const broken = endpoint([
    { status: 200, json: async () => JSON.parse("{") },
    { status: 200, body: recorded },
  ]);
