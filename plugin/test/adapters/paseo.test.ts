...
test("what the plugin sets as a seat is created goes back to Paseo: its config and its env", async () => {
  ...
  const server = {
    before: (name: string, handler: Before) => befores.set(name, handler),
    on: () => () => {},
    handle: () => {},
  } as unknown as PluginServerContext;
  const hooks = {
    create: (config: AgentConfig, env: Record<string, string>) => ({
      config: { ...config, model: "m" },
      env: { ...env, SEATWORKS_DESK_KEY: "k" },
    }),
  } as unknown as HostHooks;
  new PaseoHost().connect(server, hooks);
  const made = befores.get("agent.create")!(
    { request: { config: { provider: "sw2-lead-claude", cwd: "/work" }, env: { KEPT: "yes" } } },
    { paseo: {} },
  );
...
test("Paseo's API reaches the host with any hook or panel call, and what waits for it goes on then", async () => {
  ...
  const settled = (host: PaseoHost) =>
    Promise.race([
      host.reached().then(() => true),
  ...
  const ways: [string, (host: PaseoHost) => void][] = [
    [
      "a hook before a seat is created",
      (host) => hooked(host, "before", "agent.create", { create: (config, env) => ({ config, env }) }),
    ],
    [
      "a hook before a session opens",
      (host) => hooked(host, "before", "agent.session_open", { sessionOpen: (request) => request }),
    ],
    ...
    [
      ...
      (host) =>
        host.answering({
          handle: (_contract: unknown, handler: Handler) => handler(undefined, { paseo: {} }),
