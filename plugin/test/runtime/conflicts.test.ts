...
test("the merge queue hands a conflict to its Peer, merges nothing as nothing, waits out a busy copy or a hold, fails a task on a crash, and is tried once more by landing", async (t) => {
  ...
  const merges = (h.runtime.desk as unknown as { services: { merges: { merge: () => Promise<void> } } }).services
    .merges;
  const crashing = t.mock.method(merges, "merge", () => Promise.reject(new Error("the disk is full")));
