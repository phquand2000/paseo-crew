...
async function heldLanes(t: { after(fn: () => void): void }) {
  ...
  await h.call(sup, "supervisor", "set_project", { gate: `if [ -f ${armed} ]; then while [ ! -f ${go} ]; do sleep 0.05; done; fi`, gateOn: "task" });
  for (const title of ["One", "Two"]) await h.call(sup, "supervisor", "open_lane", { title, outcome: title, ...scope, isolate: true });
  const lanes = ["L1", "L2"].map((id) => h.ledger().lanes[id]!);
  for (const [index, lane] of lanes.entries()) {
    await h.call(lane.lead!, "lead", "add_tasks", { tasks: [{ key: "t", title: `Side ${index}`, goal: "g", ...scope, holds: [`s${index}.txt`], parallel: true }] });
...
test("each lane merges in its own queue: a gate running on one lane's merge holds no other lane's", async (t) => {
  ...
  assert.ok(await within(5000, () => h.ledger().tasks["L2-T1"]!.status === "merged"), "the second lane's merge went by while the first lane's gate ran");
