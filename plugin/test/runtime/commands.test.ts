...
const rules = (extra: Partial<Rules> = {}): Rules => ({ ...watchPatterns(kit, kit.attention), gates: [], repeatsAt: 3, recoverWithin: 10, ...extra });
...
test("what a command makes for itself and then removes is scratch, and anything else it removes is not", () => {
  ...
  const shell = (command: string) => onDetail({ id: "c", name: "Bash", status: "completed", ended: true, detail: { type: "shell", command } } as never, rules());
  ...
  assert.equal(shell(`demo=$(mktemp -d) && rm -rf "$HOME/demo"`)[0]?.kind, "destructive", "a variable nothing here set from mktemp is not scratch");
