...
test("nothing reaches a seat that names or quotes an open incident about it, while its own words about the work do", async () => {
  ...
  const now = Date.now();
  const about = (id: string, seat: string) => ({ id, seat, where: "L1", kind: "stuck", level: "attend" as const, quote, facts: ["stuck"], opened: now, last: now, count: 1, open: true });
  ...
  assert.match((await h.call(lane.lead!, "lead", "message", { to: "L1-T1", text: "About i1: why did that happen?" })).text, refusal);
  assert.match((await h.call(lane.lead!, "lead", "message", { to: "L1-T1", text: `You hit ${quote.toUpperCase()}.` })).text, refusal);
  assert.match((await h.call(lane.lead!, "lead", "amend_task", { task: "L1-T1", why: `because of ${quote}`, goal: "g2" })).text, refusal);
  assert.match((await h.call(sup, "supervisor", "message", { to: "L1-T1", text: `See I1.` })).text, refusal);
  assert.match((await h.call(sup, "supervisor", "message", { to: "L1", text: `${quote}?` })).text, /That repeats incident I2/);
  assert.match((await h.call(sup, "supervisor", "amend_lane", { lane: "L1", why: "the Human changed it", acceptance: [`no more ${quote}`] })).text, /That repeats incident I2/);
  assert.equal((await h.call(lane.lead!, "lead", "message", { to: "L1-T1", text: "Your test run keeps failing the same way; what does the first failure say?" })).ok, true);
