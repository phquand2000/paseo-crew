...
test("a last call that never came back ends the turn too, and is not called a refusal", () => {
  const timeline = t(
    { type: "user_message", text: "go" },
    { type: "tool_call", name: "exec", status: "canceled", detail: { command: "gh api user" } },
  );
...
test("a harness names how its refusals read, and only those count as refused", () => {
  ...
  assert.equal(
    deniedCall(timeline, refused),
    undefined,
    "it failed and the seat spoke after it, so the turn did not end on that call",
  );
...
test("a call the harness refused because its input was not JSON is reported, and an ordinary failure is not", () => {
  ...
  const timeline = t(
    { type: "user_message", text: "go" },
    {
      ...
      detail: {
        type: "unknown",
        input: { __unparsedToolInput: { raw: '{"title": "pick(versions, range) to spec"' } },
        output: null,
      },
    },
    {
...
test("a malformed call is still reported when only the input it could not read survives", () => {
  const timeline = t({
    ...
    status: "failed",
    error: null,
    detail: { type: "unknown", input: { __unparsedToolInput: { raw: "{" } }, output: null },
  });
  assert.deepEqual(
    malformed(timeline, unparsed).map((call) => call.tool),
    ["done"],
  ...
  const timeline = t(
    { type: "user_message", text: "go" },
    {
      ...
      detail: {
        ...
        command: "npm test",
        exitCode: 1,
        output: "FAIL config.test.ts: could not be parsed as JSON",
      },
    },
    {
      type: "tool_call",
...
test("a malformed call belongs to the turn it was made in, and is not reported again at the end of the next", () => {
  ...
  assert.deepEqual(
    malformed(t(...turn), unparsed).map((call) => call.tool),
    ["mcp__team__open_lane"],
  );
  const later = t(
    ...turn,
    { type: "user_message", text: "now start a task" },
    { type: "tool_call", callId: "c3", name: "status", status: "completed", detail: {} },
    { type: "assistant_message", text: "Done." },
  );
  assert.deepEqual(
    malformed(later, unparsed),
    [],
    "the same call is not written to the log again every turn for the rest of the session",
  );
});
