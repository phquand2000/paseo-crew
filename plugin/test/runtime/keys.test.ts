...
test("a seat with tools is given a key as it is created: in its env, and in its team server's where the harness keeps one per seat", () => {
  const h = harness();
  const made = h.runtime.create({ provider: "sw2-lead-claude", cwd: h.root } as AgentConfig, { KEPT: "yes" });
  ...
  const other = h.runtime.create({ provider: "sw2-lead-claude", cwd: h.root } as AgentConfig, {});
  assert.notEqual(other.env[KEY], key, "each seat its own");
  const onFile = h.runtime.create({ provider: "sw2-peer-omp", cwd: h.root } as AgentConfig, {});
  assert.match(onFile.env[KEY]!, /^[0-9a-f]{48}$/, "a harness reading servers from a file shared by its seats gets the key through the env alone");
  assert.equal(h.runtime.create({ provider: "sw2-pager-claude", cwd: h.root } as AgentConfig, {}).env[KEY], undefined, "a seat with no tools has no server to give it to");
});
...
test("the desk binds a key to its agent as Paseo opens the seat, gives it back each time the seat opens again, and lets it go once archived", async () => {
  ...
  const open = (reason: "create" | "resume", env: Record<string, string> = {}) => h.runtime.sessionOpen({ agentId: "agent-9", reason, provider: "sw2-lead-claude", cwd: h.root, env });
  ...
  assert.equal(h.runtime.sessionOpen({ agentId: "agent-0", reason: "resume", provider: "sw2-lead-claude", cwd: h.root, env: {} }).env[KEY], undefined, "a seat never given one gets none");
