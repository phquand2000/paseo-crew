...
test("a Lead lays its lane out at once: tasks in the lane's copy run in turn, parallel ones beside them, each starting once what it waits for is merged", async () => {
  ...
  assert.match(
    planned.text,
    /- TAX is L1-T4 Task tax: waits for L1-T2/,
    "the lane's copy takes one writer, so the plan's tasks there run one after another",
  );
  await h.idle(lead);
  assert.doesNotMatch(
    h.agents.get(lead)!.sent.join("\n"),
    /WAITING L1-T3/,
    "the reply already said it started, so no letter says it again",
  );
  ...
  assert.deepEqual(
    ...
    ["running", "waiting"],
    "L1-T2 starts once L1-T1 is accepted, and L1-T4 still waits for it",
  );
});
...
test("a task that holds what a running task holds, and does not wait for it, is refused; one that waits is taken", async () => {
  ...
  assert.equal(
    (await h.call(opened.lead!, "lead", "add_tasks", { tasks: [task("first", ["b.txt"], { parallel: true })] })).ok,
    true,
  );
  const refused = await h.call(opened.lead!, "lead", "add_tasks", {
    tasks: [
      task("beside", ["b.txt"], { parallel: true }),
      task("next", ["b.txt"], { parallel: true, after: ["L1-T2"] }),
    ],
  });
  ...
  assert.equal(
    (
      await h.call(opened.lead!, "lead", "add_tasks", {
        tasks: [task("next", ["b.txt"], { parallel: true, after: ["L1-T2"] })],
      })
    ).ok,
    true,
  );
...
test("a Lead changes what a task beside others holds: the record and its Peer see what it holds now, and it may not take what another holds", async () => {
  ...
  const amend = (id: string, holds: string[]) =>
    h.call(lead, "lead", "amend_task", { task: id, why: "the desk named a path", holds });
  assert.match(
    (await amend("L1-T2", ["b.txt", "c.txt"])).text,
    /What it holds overlaps what L1-T3 holds at c\.txt\. Leave those paths out of L1-T2/,
  );
  assert.match(
    (await amend("L1-T2", ["b.txt", "package-lock.json"])).text,
    /A parallel task can't hold package-lock\.json/,
  );
...
test("a task's brief names what is written beside it and what each holds, and one in the lane's copy only the tasks in copies of their own", async () => {
  ...
  await h.call(lead, "lead", "add_tasks", {
    tasks: [
      task("n", ["n.txt"]),
      task("b", ["b.txt"], { parallel: true }),
      task("c", ["c.txt"], { parallel: true }),
      task("w", ["w.txt"], { after: ["b", "c"] }),
    ],
  });
  ...
  assert.match(
    ...
    "L1-T5 comes after it, so it is not beside it",
  );
  assert.match(
    brief("L1-T4"),
    /Beside you, in copies of their own or the lane's, each merged into the lane branch once accepted: L1-T1 \(Clean build\); L1-T2 \(Task n\); L1-T3 \(Task b\) holds b\.txt\./,
  );
