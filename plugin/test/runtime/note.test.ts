...
test("a Lead keeps a page in a folder its role writes under the project's state, and nowhere else", async () => {
  ...
  assert.match(
    (await note({ kind: "gates", name: "x.md", text: "t" })).text,
    /gates is no folder you keep pages in: plans, council, ultra-review, repo-refresh\./,
  );
  assert.match(
    (await note({ kind: "..", name: "ledger.json", text: "{}" })).text,
    /\.\. is no folder you keep pages in/,
  );
  assert.match(
    (await note({ kind: "plans", name: "../ledger.json", text: "{}" })).text,
