...
test("a Peer whose task is accepted stays until its Lead releases it, and never takes another task: the next one starts a Peer of its own", async () => {
  ...
  assert.equal(
    (await h.call(lead, "lead", "add_tasks", { tasks: [task("u", "Second", "a.txt", { after: ["L1-T1"] })] })).ok,
    true,
  );
...
test("the Lead releases the Peer kept from an accepted task; not while the task runs", async () => {
  ...
  const lead = lane.lead!;
  assert.match(
    (await h.call(lead, "lead", "release", { task: "L1-T1" })).text,
  ...
  assert.match(
    (await h.call(lead, "lead", "release", { task: "L1-T1" })).text,
    /The Peer kept from L1-T1 is gone already\./,
  );
  ...
  assert.match(
    (await h.call(lead, "lead", "release", { task: review.id })).text,
    new RegExp(`${review.id} is a review: its reviewer goes when you cut it\\.`),
  );
});
...
test("a parallel task's Peer stays in its own copy once merged, until its Lead releases it and the copy goes with its branch", async () => {
  ...
  assert.equal(
    h.git(h.root, "branch", "--list", side.branch!).trim(),
    "",
    "and so is its branch, merged into the lane",
  );
});
...
test("a merged parallel task's copy goes once its kept Peer is gone, whether the Human archived it or the lane closed", async () => {
  ...
  assert.equal(
    (await other.h.call(other.sup, "supervisor", "drop_lane", { lane: "L1", reason: "not wanted" })).ok,
    true,
  );
...
test("landing lets a kept parallel Peer go after the turn it is in, and its merged branch goes with its copy", async () => {
  ...
  assert.equal(
    ...
    "",
    "the lane branch goes at once, being all under its landed ref",
  );
  ...
  assert.equal(
    h.git(h.root, "branch", "--list", side.branch!).trim(),
    "",
    "its work is in the landed lane, so its branch goes",
  );
});
...
test("the Lead sends an accepted task back to the Peer kept on it, which fixes it on top of what was accepted", async () => {
  ...
  assert.deepEqual(
    [task.status, task.peer, h.git(lane.worktree!, "branch", "--show-current").trim()],
    ["rework", peer, task.branch],
...
test("an accepted task goes back only to its Peer while kept, and not into a copy another task holds", async () => {
  ...
  await h.call(lead, "lead", "add_tasks", { tasks: [task("u", "Second")] });
  assert.match(
    (await h.call(lead, "lead", "rework", { task: "L1-T1", text: "x" })).text,
    /L1-T2 holds the lane's working copy/,
  );
  ...
  assert.match(
    (await h.call(lead, "lead", "rework", { task: "L1-T1", text: "x" })).text,
    /The Peer on L1-T1 is gone: add a task for what must change\./,
  );
  assert.equal(h.ledger().tasks["L1-T1"]!.status, "merged");
});
...
test("a hand-back sent back to a Peer that is gone stays as it was, for its Lead to cut", async () => {
  ...
  assert.match(
    (await h.call(lane.lead!, "lead", "rework", { task: "L1-T1", text: "x" })).text,
    /The Peer on L1-T1 is gone; cut the task and start a new one\./,
  );
