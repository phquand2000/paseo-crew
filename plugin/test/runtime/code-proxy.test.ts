...
test("the IDE server carries the navigation rule, lists only the role's tools and hides the project argument", async () => {
  ...
  try {
    ...
    assert.equal(
      ...
      "the catalog's rule, as the catalog words it",
    );
    ...
    await assert.rejects(
      code.call("ide_refactor_rename", {}),
      "a tool the role was not given is the protocol's own error",
    );
  } finally {
    await code.stop();
    ide.close();
...
test("an IDE call is pinned to the working copy, a preset that opens it does so on first use, and a switched-off tool is reported", async () => {
  ...
  const code = await proxy(
    cwd,
    opening(ideConfig(ide.url, ["ide_find_references", "ide_find_symbol", "ide_diagnostics"])),
  );
  try {
    ...
    const catalogReply = (backendSaid: string, tool: string) =>
      (entry("intellij-index").proxy.errors as { when: string; reply: string }[])
        .find((error) => new RegExp(error.when, "i").test(backendSaid))!
...
test("files changed outside the IDE are synced before the next call, and nothing is synced when nothing changed", async () => {
  ...
  try {
    ...
    await code.call("ide_find_references", {});
    assert.deepEqual(
      syncs().map((call) => call.args.paths),
      [["new.ts"]],
    );
    await code.call("ide_find_references", {});
...
test("code search lists the backend's tool with the catalog's description, hides the pinned argument and runs against the working copy", async () => {
  ...
  const code = await proxy(cwd, {
    name: "code-search",
    label,
    instructions,
    tools: ["search"],
    ...spec,
    backend: { type: "stdio", command: [process.execPath, fakeSemble()] },
  });
  try {
    const listed = await code.client.listTools();
    assert.deepEqual(
...
test("the desk opens a copy it takes with the shipped entry, routed through an open project, and closes that copy alone", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      ide.calls.map((call) => [call.args.path, call.args.project_path]),
      [
        ["/slots/S1", undefined],
...
test("a server that is slow to start does not hold the tool list for the whole call budget, and is not left running", async () => {
  ...
  try {
    ...
    assert.match(
      only.description,
      /not reachable/,
      "and the seat is told the server is not there, which a configured description used to hide",
    );
    assert.equal(
      only.inputSchema.additionalProperties,
      true,
      "with a schema that does not refuse the arguments it would be called with",
    );
  } finally {
  ...
  try {
    assert.ok(
      ...
      "a backend that ignores its input closing is stopped with its proxy, not left running for good",
    );
  } finally {
