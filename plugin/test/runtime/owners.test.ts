...
test("a Peer's failed turn or wait for permission reaches whoever supervises once its Lead is gone, not a Lead nobody reads for", async () => {
  ...
  await h.runtime.turnEnded({ agent: { id: peer, provider: seat.provider, cwd: seat.cwd, title: seat.title }, turnId: "t-gone", outcome: { kind: "failed", error: { message: "the model is overloaded" } }, timeline: [] });
