...
test("a task beside others that holds nothing is refused, and so is one in the lane's copy that holds paths", async () => {
  ...
  assert.match(
    await refused([
      { key: "a", title: "A", goal: "g", ...scope, holds: ["src/app.ts"], parallel: true },
      { key: "b", title: "B", goal: "g", ...scope, parallel: true },
    ]),
    /B runs beside others but holds nothing/,
  );
  assert.match(
    await refused([{ key: "c", title: "C", goal: "g", ...scope, holds: ["src/app.ts"] }]),
...
test("a brief points where to start reading, not a fence, and names what a task beside others holds", async () => {
  ...
  assert.match(
    brief("L1-T1"),
    /Beside you, in copies of their own, each merged into the lane branch once accepted: L1-T2 \(Receipt\) holds src\/receipt\/\. What they write reaches your copy only as your hand-back brings the lane in/,
  );
  assert.match(
    brief("L1-T2"),
...
test("a hand-back names each file once: in what a task beside it holds before outside the lane's write set", async () => {
  ...
  await h.call(lead, "lead", "add_tasks", {
    tasks: [
      ...
      { key: "s", title: "S", goal: "g", ...scope, holds: ["c.txt", "**/*.md"], parallel: true },
    ],
  });
  ...
  await h.idle(lead);
  const notes = letters(h, lead)
    .split("\n")
    .filter((line) => line.startsWith("Note:"));
  assert.deepEqual(notes, [
    "Note: in what L1-T2 holds (c.txt, **/*.md): c.txt, notes.md.",
    "Note: outside the lane's write set (a.txt, c.txt, src/**): d.txt.",
  ]);
});
...
test("a Peer beside others is noted once for each file past what it holds: in what another holds, outside the write set, or only outside its own", async () => {
  ...
  await h.call(lead, "lead", "add_tasks", {
    tasks: [
      { key: "p", title: "P", goal: "g", ...scope, holds: ["src/p/"], parallel: true },
      { key: "q", title: "Q", goal: "g", ...scope, holds: ["src/q/"], parallel: true },
    ],
  });
  const task = h.ledger().tasks["L1-T1"]!;
  ...
  const notes = letters(h, lead)
    ...
    .filter((line) => line.startsWith("Note:"));
  assert.deepEqual(notes, [
    "Note: in what L1-T2 holds (src/q/): src/q/b.ts.",
...
test("the watch reads a Peer in the lane's copy against the lane's write set, and one beside others against what it holds", async () => {
    ...
    timeline.add(
      {
        ...
        name: "Edit",
        status: "completed",
        detail: { type: "edit", filePath: join(task.worktree!, file), oldString: "", newString: "x\n" },
      },
      `${call}-turn`,
    );
  };
  ...
  assert.deepEqual(facts.map((event) => `${peers.indexOf(event.agent)} ${event.quote.split("/").at(-1)}`).sort(), [
    "0 d.txt",
    "1 b.txt",
  ]);
});
...
test("a Peer at work in the lane's copy is told when a task starts beside it after its brief, and what that task holds", async () => {
  const { h, lane, peer } = await laneWithPeer();
  await h.call(lane.lead!, "lead", "add_tasks", {
    tasks: [{ key: "s", title: "Receipt", goal: "g", ...scope, holds: ["src/receipt/"], parallel: true }],
  });
  ...
  assert.doesNotMatch(
    h.heard(h.ledger().tasks["L1-T2"]!.peer!).join("\n"),
    /BESIDE/,
    "its own brief already names who writes beside it",
  );
  ...
  await h.call(lane.lead!, "lead", "add_tasks", {
    tasks: [{ key: "t", title: "Totals", goal: "g", ...scope, holds: ["src/totals/"], parallel: true }],
  });
  await h.idle(peer);
