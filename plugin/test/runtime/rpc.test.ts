...
test("the plugin serves the catalog, settings, projects, team and status over RPC", async () => {
  ...
  assert.deepEqual(names.sort(), [
    "paseo-crew.catalog.read",
    "paseo-crew.doctor.run",
    "paseo-crew.flow.read",
    "paseo-crew.mcp.parse",
    "paseo-crew.models.refresh",
    "paseo-crew.paths.list",
    "paseo-crew.projects.add",
    "paseo-crew.projects.candidates",
    "paseo-crew.projects.list",
    "paseo-crew.projects.remove",
    "paseo-crew.settings.read",
    "paseo-crew.settings.write",
    "paseo-crew.status.read",
    "paseo-crew.team.read",
    "paseo-crew.upkeep.clean",
    "paseo-crew.upkeep.decide",
    "paseo-crew.upkeep.migrate",
...
test("the daemon handle a panel call arrives with is kept, not thrown away", async () => {
  ...
  await call("paseo-crew.catalog.read");
...
test("a web app turns a server on for the machine and switches a role's harness for one project", async () => {
  ...
  const saved = await call("paseo-crew.settings.write", { revision: read.revision, values: { mcp: { docs: { enabled: true } } } });
  assert.equal(saved.status, "saved");
  let team = await call("paseo-crew.team.read");
  assert.deepEqual(team.roles.lead.mcp, ["ide", "docs"]);
  assert.match(team.roles.lead.rules, /Look library APIs up in the docs\./);
  assert.equal(team.roles.lead.provider, "crew-lead-claude");

  const state = join(HOME, ".local/share/paseo-crew/projects/shop-abc123");
  ...
  const status = await call("paseo-crew.status.read", { project: "shop-abc123" });
...
test("a project can be registered by its path before any agent has run in it", async () => {
  const { call } = served();
  const root = realpathSync(mkdtempSync(join(tmpdir(), "crew-rpc-project-")));
  execFileSync("git", ["init", "-q", root]);
  mkdirSync(join(root, "src"), { recursive: true });
  const added = await call("paseo-crew.projects.add", { root: join(root, "src") });
...
test("attaching a project is undone by detaching it, unless work is still running in it", async () => {
  const { call } = served();
  const root = realpathSync(mkdtempSync(join(tmpdir(), "crew-rpc-attach-")));
  execFileSync("git", ["init", "-q", root]);
  ...
  assert.match((await call("paseo-crew.projects.remove", { project: added.slug })).error, /has been seen/);
});

test("the projects a setup screen may offer leave out worktrees, gone directories and the ones already set up", async () => {
  const { call } = served();
  const repo = realpathSync(mkdtempSync(join(tmpdir(), "crew-rpc-live-")));
  execFileSync("git", ["init", "-q", repo]);
  execFileSync("git", ["-c", "user.email=t@t", "-c", "user.name=t", "commit", "-q", "--allow-empty", "-m", "init"], { cwd: repo });
  const linked = join(realpathSync(mkdtempSync(join(tmpdir(), "crew-rpc-linked-"))), "wt");
  execFileSync("git", ["worktree", "add", "-q", "-b", "side", linked], { cwd: repo });
  const plain = realpathSync(mkdtempSync(join(tmpdir(), "crew-rpc-plain-")));
  const ours = join(HOME, ".local/share/paseo-crew/worktrees/shop-ef484b/S0");
  mkdirSync(ours, { recursive: true });
  const taken = realpathSync(mkdtempSync(join(tmpdir(), "crew-rpc-taken-")));
  execFileSync("git", ["init", "-q", taken]);
  await call("paseo-crew.projects.add", { root: taken });

  const roots = [repo, linked, plain, ours, join(repo, "nowhere"), taken];
  assert.deepEqual(await call("paseo-crew.projects.candidates", { roots }), [repo]);
});

test("a pasted server is understood whatever dialect it is written in", async () => {
  const { call } = served();
  const nested = await call("paseo-crew.mcp.parse", {
  ...
  const claudeStyle = await call("paseo-crew.mcp.parse", {
  ...
  const remote = await call("paseo-crew.mcp.parse", { text: JSON.stringify({ type: "remote", url: "https://mcp.example/mcp", headers: { Authorization: "Bearer x" } }) });
  ...
  const fromReadme = await call("paseo-crew.mcp.parse", {
  ...
  assert.match((await call("paseo-crew.mcp.parse", { text: "not json" })).error, /not JSON/);
  assert.match((await call("paseo-crew.mcp.parse", { text: JSON.stringify({ type: "local" }) })).error, /needs a command/);
  assert.match((await call("paseo-crew.mcp.parse", { text: JSON.stringify({ command: "npx", env: { KEY: { from: "keychain" } } }) })).error, /env gives KEY/, "what has no text form is named, not dropped");
});

test("a server pasted into the settings reaches the seats, and a shipped one can be removed", async () => {
  const { call } = served();
  const read = await call("paseo-crew.settings.read");
  const saved = await call("paseo-crew.settings.write", {
  ...
  const team = await call("paseo-crew.team.read");
...
test("a folder that is there and cannot be read is a refusal, not a rejected call", async () => {
  const { call } = served();
  const root = mkdtempSync(join(tmpdir(), "crew-noread-"));
  mkdirSync(join(root, "locked"));
  chmodSync(join(root, "locked"), 0o000);
  try {
    const answer = await call("paseo-crew.paths.list", { path: join(root, "locked") });
...
test("a project detached in this session can be attached again, and the desk's half of a second setup keeps the layer", async () => {
  ...
  const root = realpathSync(mkdtempSync(join(tmpdir(), "crew-again-")));
  const added = await call("paseo-crew.projects.add", { root });
  ...
  const again = await call("paseo-crew.projects.add", { root });
  assert.equal(again.slug, added.slug, "the same repository is the same project");
  const still = await call("paseo-crew.settings.read", { project: added.slug });
  ...
  assert.deepEqual(await call("paseo-crew.projects.remove", { project: added.slug }), { removed: added.slug });
  const back = await call("paseo-crew.projects.add", { root });
  assert.equal(back.slug, added.slug);
  assert.ok(
    (await call("paseo-crew.projects.list", {})).some((entry: { slug: string }) => entry.slug === added.slug),
    "an attach that reports a slug has to be an attach the rest of the plugin can find",
  );
  assert.equal((await call("paseo-crew.settings.read", { project: added.slug })).status, "ready");
  runtime.dispose();
});

test("the setup screen can walk this machine's folders to find a repository", async () => {
  const { call } = served();
  const root = realpathSync(mkdtempSync(join(tmpdir(), "crew-rpc-browse-")));
  mkdirSync(join(root, "plain"), { recursive: true });
  execFileSync("git", ["init", "-q", join(root, "repo")]);

  const listed = await call("paseo-crew.paths.list", { path: root });
  ...
  const inside = await call("paseo-crew.paths.list", { path: join(root, "repo") });
  assert.equal(inside.repository, true);
  assert.match((await call("paseo-crew.paths.list", { path: join(root, "nowhere") })).error, /is not a directory/);
});

test("the sensor's key is written from the panel, never read back into it, and forgotten only when asked", async () => {
  const { call } = served();
  const file = join(HOME, ".local/share/paseo-crew/settings.json");
  const onDisk = () => JSON.parse(readFileSync(file, "utf8")) as { sensor?: { key?: string }; rules?: string };

  const read = await call("paseo-crew.settings.read");
  const saved = await call("paseo-crew.settings.write", { revision: read.revision, values: { ...read.values, sensor: { key: "sk-or-secret" } } });
  assert.equal(saved.status, "saved");
  ...
  const back = await call("paseo-crew.settings.read");
  ...
  const else_ = await call("paseo-crew.settings.write", { revision: back.revision, values: { ...back.values, rules: "watch the watch" } });
  assert.equal(else_.status, "saved");
  assert.equal(onDisk().sensor?.key, "sk-or-secret");
  assert.equal(onDisk().rules, "watch the watch");

  const state = join(HOME, ".local/share/paseo-crew/projects/sensor-abc123");
  mkdirSync(state, { recursive: true });
  writeFileSync(join(state, "meta.json"), JSON.stringify({ root: "/work/sensor", slug: "sensor-abc123" }));
  const project = await call("paseo-crew.settings.read", { project: "sensor-abc123" });
  ...
  assert.doesNotMatch(JSON.stringify(await call("paseo-crew.team.read")), /sk-or-secret/);
  assert.doesNotMatch(JSON.stringify(await call("paseo-crew.team.read", { project: "sensor-abc123" })), /sk-or-secret/);

  const { sensor: _gone, ...without } = (await call("paseo-crew.settings.read")).values as { sensor?: unknown };
  const forgotten = await call("paseo-crew.settings.write", { revision: (await call("paseo-crew.settings.read")).revision, values: without });
...
test("a settings file that will not parse is reported without quoting what it holds", async () => {
  ...
  const file = join(HOME, ".local/share/paseo-crew/settings.json");
  const read = await call("paseo-crew.settings.read");
  assert.equal((await call("paseo-crew.settings.write", { revision: read.revision, values: { ...read.values, sensor: { key: "sk-or-secret" } } })).status, "saved");
  ...
  const shown = [
    await call("paseo-crew.settings.read"),
    await call("paseo-crew.settings.write", { revision: read.revision, values: { rules: "x" } }),
    await call("paseo-crew.team.read"),
    await call("paseo-crew.doctor.run"),
  ];
...
test("the word that stands for the key is never itself written, and a refused or stale save leaves it where it was", async () => {
  ...
  const empty = await call("paseo-crew.settings.read");
  assert.equal((await call("paseo-crew.settings.write", { revision: empty.revision, values: { rules: "one", sensor: { key: KEPT } } })).status, "saved");
  ...
  const last = await call("paseo-crew.settings.read");
  const { sensor: _gone, ...without } = last.values as { sensor?: unknown };
  assert.equal((await call("paseo-crew.settings.write", { revision: last.revision, values: without })).status, "saved");
});
