...
test("a call its harness file names as no call of the seat's own is kept apart, on that harness only", () => {
  const stdin = { type: "tool_call", callId: "c1", name: "terminal", status: "completed", detail: { type: "plain_text", text: "y\n" } };
  assert.equal(new Window(kit.harnesses.codex!.timeline).add(row(stdin)).call?.pseudo, true, "Codex writing to a command's stdin is not a command");
