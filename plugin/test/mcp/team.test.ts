...
test("a seat's harness is shown its tools with the desk's choices, and its calls are carried out as the keyed seat's, or refused for an unknown key", async (t) => {
  ...
  assert.deepEqual(fields.role!.enum, ["peer", "backup-peer"], "the desk's choices as enums");
...
test("a seat started before a tool was added is told the list changed and then sees it", async (t) => {
  ...
  copyFileSync(join(dirname(TEAM), "..", "package.json"), join(root, "package.json"));
  symlinkSync(join(dirname(TEAM), "..", "node_modules"), join(root, "node_modules"));
  ...
  const client = new Client(
    { name: "probe", version: "0" },
    { listChanged: { tools: { autoRefresh: false, debounceMs: 0, onChanged: () => told++ } } },
  );
  const transport = new StdioClientTransport({
    command: process.execPath,
  ...
  assert.deepEqual(await names(), ["status"]);
  writeFileSync(join(dir, "tools.json"), JSON.stringify({ supervisor: [tool("status"), tool("amend_lane")] }));
  await until(() => told > 0, "the seat is told the list changed");
  assert.deepEqual(await names(), ["status", "amend_lane"]);
});
