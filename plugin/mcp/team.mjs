...
let tools = read("tools.json")[toolSet] ?? [];
...
serveStdio(async () => {
  ...
  const register = (tool) => {
    ...
    held.set(tool.name, { tool, schema, entry });
  };
  ...
  desk.changed = () => {
    for (const shown of held.values()) {
      const schema = schemaOf(shown.tool);
  ...
  watchFile(join(here, "tools.json"), { interval: Number(process.env.SEATWORKS_TOOLS_POLL_MS ?? 5000) }, () => {
    ...
    for (const tool of tools) {
      const shown = held.get(tool.name);
      if (!shown) register(tool);
      else if (JSON.stringify(shown.tool) !== JSON.stringify(tool)) {
        shown.tool = tool;
        shown.schema = schemaOf(tool);
        shown.entry.update({ title: tool.title, description: tool.description, annotations: tool.annotations, paramsSchema: fromJsonSchema(JSON.parse(shown.schema), unchecked) });
      }
    }
    for (const [name, shown] of held) if (!tools.some((tool) => tool.name === name)) (shown.entry.remove(), held.delete(name));
  }).unref();
  return server;
});
