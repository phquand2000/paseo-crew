...
export type WatchView = { on: boolean; telling: boolean; readings: number; cost: number; seats: WatchSeat[]; incidents: { open: number; held: number }; trouble: WatchTrouble[] };
export type FlowView = { project: string; at: number; revision: string; supervisors: FlowSeat[]; lanes: FlowLane[]; moreLanes: number; asks: FlowAsk[]; watch: WatchView | null };
