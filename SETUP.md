...
| `guides/` | the shapes a seat reads when it needs one: `WORKSPACE_PROTOCOL.md`, `DIRECTIVE.md`, `FEATURE_INTAKE.md`, `PLANS.md`, `ADR.md`, `REVIEW.md`, `BRIEF.md`, `STRUCTURAL_LENSES.md` | replaced, except `WORKSPACE_PROTOCOL.md` once its placeholders are filled in |
...
1. Replace each `<hint>` in `AGENTS.md` and `.seatworks/guides/WORKSPACE_PROTOCOL.md` with the
   Human's decision, or delete the line to keep the Lead's default; the hint says what it wants.
   Committing is the Human's call. List what is open with:
