...
The skill produces `records/strategy/CONCERN.md` in the kit, built from
...
2. **Gather about five inputs.** Collect recent documents in which a real tradeoff on this
   ...
   in `records/directives/`. Prefer recent documents and ones from different projects. If a repository is
...
6. **Write the enforcing actions.** For each policy, list the concrete actions that make it
   ...
   change to a kit prompt or skill through the protocol-patch skill, or a lens used in reviews.
...
8. **Write the file.** Save `records/strategy/CONCERN.md` from the template, with the status `draft`, the
   inputs, and a review date about two months out. Seats in this kit have no scheduler, so the
...
10. **Relay the parts that bind Leads.** Send each affected Lead an `OWNER DIRECTIVE:` with the
    policies and actions that apply to its project, stated as constraints. Changes to kit prompts
