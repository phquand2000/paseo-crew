...
description: "Turns a Human request into a settled owner directive: interviews the Human in rounds of numbered questions with recommended answers, capturing the problem story, outcome, appetite, no-gos, success check, and decisions reserved for the Human. Use when the Human brings new work or changes an outcome, including when the Human is away and the questions go out as a questionnaire."
---

# Intent interview

Use this skill to turn the request given with this skill into an owner directive that a Lead can
act on without coming back to the Human.

The skill produces the directive text, filled in from
[references/directive-template.md](references/directive-template.md). Paths in this skill are
relative to this skill's directory, except paths starting with `claude/`, `records/directives/`, or
`records/questionnaires/`, which are relative to the kit. You send the directive to a Lead and keep a
copy at `records/directives/YYYY-MM-DD-SLUG.md` in the kit: a retrospective later needs the exact text
the Lead received, and the Lead's transcript goes away when the Lead is archived.
...
3. Choose the mode. If the Human is here, interview live. If the Human is away, or asks to answer
   later, go to Async mode. **Done** when the mode is chosen.
...
1. **Get the problem story.** Ask about the most recent real occurrence: when it last happened,
   ...
   rather than "would you want", because questions about hypotheticals get optimistic, polite
   answers. **Done** when the story names a concrete episode with a date or a cost.
2. **Separate the outcome from any pre-chosen solution.** When the request names a solution
   ("add a cache", "rewrite in Go"), ask what the solution would give the Human once it works, and
   record that as the outcome. Keep the solution only as a constraint, and only if the Human says
   it's fixed and why. Framing belongs to the Lead, and a solution written into the directive
   takes that judgment away from the Lead and its Peers. **Done** when the outcome describes an
   observable change without naming an implementation, or the Human has fixed the solution and
   stated the reason.
...
4. **Ask the whole frontier in one round.** Number the questions, give your recommended answer
   ...
   same round waits for the next round. Use this format:
...
Stop asking once every field is filled and the Human has confirmed the readback. Questions
after that point are yours to look up or the Lead's to settle.
...
- **Appetite:** ask what the outcome is worth, meaning the time or cost the Human will spend
  ...
  scope to fit, while an estimate lets scope grow to fill it.
- **No-gos:** what is explicitly out, even where it looks cheap to include. Without them, a Lead
  widens scope in good faith.
- **Known hard parts:** traps the Human already suspects. Name them so that the Lead looks at
  them early; leave solving them to the Lead.
- **Reversibility:** mark each decision as reversible or irreversible. Irreversible decisions are
  those that are expensive or impossible to undo: deleting data, changing a public API or a
  contract other projects consume, migrations, spending money, external commitments, and
  anything that leaves this machine. Reversible decisions go to the Lead. Irreversible ones are
  reserved for the Human, each with the point at which the Lead comes back. A heavy process on
  reversible decisions only slows the Lead down; a light one on irreversible decisions produces
  outcomes the Human can't undo.
- **Success check:** something a person can run or observe: a command and its expected output,
  a metric with a threshold, or a named screen someone looks at. "It works well" is not a check.

## Async mode

1. Write a questionnaire to `records/questionnaires/YYYY-MM-DD-SLUG.md` in the kit, using
   [references/questionnaire-template.md](references/questionnaire-template.md). Put the most
   important question first, meaning the one whose answer changes the most other answers,
   because the Human may answer only the first few. Ask one idea per question, give your
   recommended answer, and leave an empty answer stub. **Done** when every frontier question and
   every empty template field has a question in the file.
...
3. When the Human has answered, read the file, fold in the answers, and continue with live rounds
   for whatever is left. A question the Human hasn't answered stays unsettled, and unsettled
   questions don't go to a Lead. **Done** when every field is filled and the Human has confirmed
   the readback.
...
3. Send it. Re-read the agent IDs with `list_agents` first.
   ...
   The text starts with `OWNER DIRECTIVE:`. **Done** when `get_agent_activity` shows that the Lead
   received it.
