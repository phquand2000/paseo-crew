...
relative to this skill's directory; every other path is relative to the kit.
...
3. **Sample the activity sparingly.** For each Lead, call `get_agent_activity` limited to the
   ...
   - signs of an unhealthy Lead, from the Observing list in `claude/SUPERVISOR.md`.
...
4. **Read each repository.** Run `git -C REPO log -1 --format=%ad --date=short` and
   ...
   `records/directives/`. **Done** when each project has an outcome, a last commit date, and a last
   activity date.
...
6. **Check the kit's own items.** Flag strategies in `records/strategy/*.md` past their review date,
   `records/safety/seat-matrix.md` older than the last provider or guard change, integration maps in
   `records/integration/*.md` with a phase open longer than its appetite, and notebook entries whose
