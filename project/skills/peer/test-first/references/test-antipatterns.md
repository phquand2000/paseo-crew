...
|---|---|---|---|
| Minted API | The test calls a type, field, function, route, or table that production code lacks and the brief's Interfaces don't name. | The test invents the contract; later code bends to it, and the next agent reads it as the spec. | Settle or build the contract first; if it isn't yours, report `BLOCKED` with the missing names. |
...
| Order-dependent | Tests share mutable fixtures or pass only in one order. | A change elsewhere fails unrelated tests. | Give each test its own state, created and cleaned up inside it. |
