...
description: "Change performance on evidence: measure a baseline on the claimed path, profile to find the actual bottleneck, change one thing, re-measure with the same method, keep or revert on the numbers, and add a guard where the repository has a place for one. Use when a brief asks to make something faster, smaller, or cheaper, or to investigate a slowdown."
---

# Performance change

Use this skill to change performance on evidence: a baseline on the claimed path, one change at a time, and a decision to keep or revert made on the numbers.

## Pin the claim and the method

1. Name the claimed path, meaning the production entry point and workload the brief cares about (an endpoint, a command, a render, a frame loop, a query), and the metric (median or p95 latency, throughput, allocations, peak memory, bundle size, query count).
...
3. Check that the method exercises the claimed path. A benchmark over a mock, a fixture, or a neighboring code path measures something else; report that as a finding before optimizing against it.
...
Profile before you choose a change, using a CPU profile, an allocation profile, a query log or plan, or a trace. Name the one thing that dominates: a function, query, lock, allocation site, or round trip. Typical culprits:
...
3. Measure again with the same command, inputs, sample count, and cache state as the baseline, and compare the difference with the spread.
...
Done when every attempt, kept or reverted, is in the attempt ledger described below. The ledger keeps a dead idea from being tried again.

## Add a guard

If the repository already has a place for one, such as a benchmark suite, a size or performance budget file, or a CI budget step, add a benchmark or budget for the metric you moved, using the same method. If it has none, name the guard you would propose under Unknown / risk instead of building a framework for it.
...
A number without its method, inputs, and machine can't be compared, so give all three. Put abstraction overhead, paths you couldn't measure, and differences between your machine and production under Unknown / risk.
