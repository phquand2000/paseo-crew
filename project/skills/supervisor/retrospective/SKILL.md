...
an entry in `.seatworks/NOTEBOOK.md`, in that file's entry format. You can't fix a model, but you can fix
...
2. **Gather the timeline before interpreting anything.** A cause proposed before the timeline
   ...
   `.seatworks/records/timelines/YYYY-MM-DD-SLUG.md`. **Done** when every event you will cite later has a
   row with a source.
3. **Reconstruct what each agent could see.** For each decision point in the timeline, record:
   ...
   - the context: how long the session was, whether it had compacted, and what the agent had
     read;
   - the tools: the provider's `disallowedTools`, `$SEATWORKS_KIT/pi/extensions/peer-guard.ts` for Peers, and
...
5. **Check earlier actions.** Find notebook entries on the same pattern with the status
   `applied SHA`. Did the behavior change after that commit's date? An action that didn't
...
6. **Decide on one or two actions.** Give each action a type:
   ...
   trigger.
7. **Write the notebook entry.** Use the notebook's entry format exactly:
