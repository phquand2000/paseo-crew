...
The skill produces a commit in this repository, made by the Lead on an owner directive (or, for
a kit surface, a diff for the Human), the notebook entry's `Status` line set to `applied SHA`,
and a list of agents to archive. Paths starting with `references/` are relative to this skill's
...
2. **Choose the narrowest owning surface**, so the change reaches only the agents that need it:
   ...
   retrying. If it's a procedure used now and then, make it a skill, so the prompts loaded on
   every turn stay short. **Done** when you have chosen one surface and can say why each
...
4. **Write the change.** Follow `$SEATWORKS_KIT/WRITING_GUIDE.md`, and also:
   ...
   - Put the reason in the rule's text. Put the removal trigger in an HTML comment beside the
     rule in the Claude seat prompts (`SUPERVISOR.md`, `LEAD.md`), and in the notebook entry for
   ...
   **Done** when the diff exists in the working tree, or as text for a kit surface. Don't commit
   yet.
...
6. **Pressure-test the change on fresh seats.** Follow [references/rehearsal.md](references/rehearsal.md):
   ...
   old text, then on another with the new text, and compare. Seats read the prompts through
   symlinks into `.seatworks/` in the working tree, so run the old text before you apply the
   diff, or with the diff stashed. **Done** when the old seat reproduces X, the new seat does Y,
   and you have quotes from both. If the old seat doesn't reproduce X, the scenario is wrong or
   the cause isn't in the prompt; go back to step 1. If the new seat still does X, or overshoots
   into the opposite extreme, revise the diff and rerun.
...
8. **Ship the change.** Run `fish $SEATWORKS_KIT/setup/setup-seats.fish --check` and fix
   whatever it reports. Have the Lead commit only the changed `.seatworks/` files, with a message
   naming the notebook entry, and set the entry's `Status` line to `applied SHA`.
   - For a repository surface, including its `.seatworks/`, send the Lead an `OWNER DIRECTIVE:`
     ...
     commit lands.
   - For a deny list or guard change, ask the Human to run
     `fish $SEATWORKS_KIT/setup/setup-seats.fish` and then `paseo reload`, because that step
     writes the seat profiles and `~/.paseo/config.json`, outside the kit.
...
9. **Retire the old versions.** Running agents keep the text they started with. Run
   `list_agents`, and give the Human every agent that started before the commit and reads the
   ...
   Peers. **Done** when the Human has the list and the agreed agents are archived.
