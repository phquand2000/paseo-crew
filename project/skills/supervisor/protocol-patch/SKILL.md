...
The skill produces a commit in this repository, made by the Lead on an owner directive (or, for a kit
surface, a diff for the Human), the notebook entry's `Status` line set to `applied SHA`, and a list of agents to archive.
...
3. **Check what already covers it.** Search the chosen surface and its neighbors for the concept,
   for example `grep -rn -i 'KEYWORD' AGENTS.md .seatworks/ $SEATWORKS_KIT/pi/ $SEATWORKS_KIT/project/`. If a line already covers
...
4. **Write the change.** Follow `WRITING_GUIDE.md`, and also:
   ...
   **Done** when the diff exists in the working tree, or as text for a kit surface.
   Don't commit yet.
...
6. **Pressure-test the change on fresh seats.** Follow [references/rehearsal.md](references/rehearsal.md):
   ...
   into `.seatworks/` in the working tree, so the order matters: run the old text before you apply the diff,
...
8. **Ship the change.** Run `fish $SEATWORKS_KIT/setup/setup-seats.fish --check` and fix whatever it reports.
   Have the Lead commit only the changed `.seatworks/` files, with a message naming the
   notebook entry, and set the entry's `Status` line to `applied SHA`.
   - For a repository surface, including its `.seatworks/`, send the Lead an `OWNER DIRECTIVE:`
     ...
     commit lands.
   - For a deny list or guard change, ask the Human to run `fish $SEATWORKS_KIT/setup/setup-seats.fish` and then
