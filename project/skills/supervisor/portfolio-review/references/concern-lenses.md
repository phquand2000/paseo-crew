...
Filters the Human can ask a review to apply. Keep what the named lens watches, and give the
Human one line for anything you notice under another lens instead of acting on it; the Leads
keep acceptance.

## Architecture

- Watches: schemas, module and service boundaries, long-term design, and consistency across
  related workspaces.
- Signals: a change to a decide-first seam in `AGENTS.md`; a new public symbol or contract with
  no decider; `REOPEN_REQUEST` at the `foundation` or `API` layer; one design argued twice.
...
- Watches: whether the work still serves the directive's outcome, and whether scope, no-gos,
  and the success check still hold.
...
- Watches: permissions, irreversible actions, sensitive surfaces (credentials, personal data,
  money), and what each seat can reach.
...
- Skills: pre-mortem. What a seat can reach is kit maintenance: ask the Human to run the kit's
  seat-safety-review.
...
- Signals: appetite spent with the outcome unmet; a stale project; a Lead waiting on a dead or
  out-of-quota Peer; heartbeats that outlived their task; over three review rounds on one task.
...
- Skills: portfolio-review's "Map a seam between projects".
