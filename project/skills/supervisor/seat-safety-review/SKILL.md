...
The skill produces `.seatworks/records/safety/seat-matrix.md`, updated in place so that git keeps the
...
`scripts/` are relative to this skill's directory; every other path is relative to the repository root.
...
[scripts/inventory.sh](scripts/inventory.sh) with `KIT=$SEATWORKS_KIT`, which prints only names and presence.
...
1. **List every seat, including providers outside the kit.** Compare
   `$SEATWORKS_KIT/examples/paseo-providers.json` (the intended setup) with `~/.paseo/config.json` (the actual
...
2. **Fill in each seat's capabilities.** For each row, record:
   - Tools: the runtime's tools minus the provider's `disallowedTools` (for Claude seats) or
     minus the patterns in `$SEATWORKS_KIT/pi/extensions/peer-guard.ts` (for the Peer); Paseo tools, from
...
6. **Choose a fix for each flag.** Prefer the cheapest fix that removes a whole leg:
   ...
   risk.
7. **Write the matrix.** Save `.seatworks/records/safety/seat-matrix.md` from the template: the date, the commands
