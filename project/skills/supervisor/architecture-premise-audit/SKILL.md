...
description: "Audits a whole project read-only for a wrong system archetype: an expected capability map from the product, compared with the observed one, and one verdict. Use when the Human explicitly asks for a broad premise audit, not ordinary design review."
...
- Derive what the product needs before you treat the repository's terminology, architecture docs,
  tests, or benchmarks as authoritative, because a repository's names describe what it intended,
  not what the product requires.
...
- Ask the Human only when one missing fact would reverse the verdict and you can't bound it with
  a stated assumption.
- For realtime or multiplayer domains, build the expected map from established domain
...
2. **Build the expected map.** From the product's needs and established mechanisms in its domain,
   ...
   repository's decomposition doesn't become your premise. **Done** when each responsibility has
   an owner and a scaling variable.
...
9. **Write the report.** Lead with the verdict, then include only the sections that support it:
   ...
   architecture and the machinery removed or relocated; the counterargument and falsifier for
   each serious finding; a `STOP_OPTIMIZING` list and a `PROBABLY_JUSTIFIED` list; and the
   prioritized decisions with realistic fitness scenarios. Give your best evidence-backed
   judgment rather than an unranked list of options. **Done** when the report is saved at
   `.seatworks/records/audits/REPO-YYYY-MM-DD.md`.
