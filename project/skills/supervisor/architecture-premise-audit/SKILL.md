...
The skill produces an audit report at `.seatworks/records/audits/REPO-YYYY-MM-DD.md`, with one verdict
...
9. **Write the report.** Lead with the verdict, then include only the sections that support it:
   ...
   an unranked list of options. **Done** when the report is saved at `.seatworks/records/audits/REPO-YYYY-MM-DD.md`.
...
11. **Relay the decision.** Send the Lead an `OWNER DIRECTIVE:` with the decision as an outcome and
    ...
    from its own repository rather than a file in `.seatworks/`. **Done** when `get_agent_activity` shows
