...
Write one route and its reason into the ExecPlan's Direction section. What a route's leftovers
cost later is catalogued under Migration and proof in
...
- **Hard cut**: `AGENTS.md` sets a hard-cut policy, typically a pre-release product with exactly
  one live contract. Follow `references/hard-cut.md` instead of the rest of this skill's routes.
