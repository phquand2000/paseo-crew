...
For a project that keeps its agent instructions and plans out of git, two values in
...
A role that can `work` or `write` is also granted the repository's git directory: a lane's copy keeps
its index and refs there, and a sandboxed agent could not commit without it.
