...
| `report` | Reports to the Supervisor. With `ready`, it first lets the merges accepted before it land, is refused while a Peer is mid-turn in the lane's copy, and runs the lane gate, and the report carries what the lane's reviews leave standing: no review of the whole lane, a latest review that did not accept, a task accepted over its own review's changes, or handed back again after them and accepted with no review since. Where review changes stand that nothing on record answers, its `Next:` asks the Supervisor to check with the Lead before landing. `land_lane` gives the same as evidence |
...
| `done` | Hands the task back to the Lead, with a file; the commit is read from the branch, and the files the task changed are named in it, counting in the lane's copy only the task's own commits, not the merges of tasks beside it. A parallel task has its lane brought into its copy first, so the gate runs on what the lane would become; conflicts there stop the hand-back and are left for its Peer to settle and commit. A note marks each file in what a task beside it holds, outside the lane's write set, or, for a parallel task, outside what it holds, a one-writer path called so; MERGED carries the same notes. A review hands back its verdict, its answer to the focus and each finding (severity, place, failure, fix), and `changes` or `reopen` needs at least one; it is refused until it answers each risk rule question its brief carries. On a per-task-gate project, it runs the gate first. It is refused once the task is accepted, queued or cut |
...
| Work moving | HANDBACK, REWORK, AMENDED, BESIDE, SETTLING, MERGED, MERGE RED, MERGE WAITS, MERGE FAILED, MERGE CONFLICT, BASE CONFLICT, REPORT, CAN LAND, CLEARED |
...
RECONCILE tells a Lead what the Supervisor sent its Peer. BESIDE tells the Peer at work in the lane's copy that a task started beside it after its brief was written, and what that task holds; a Peer that has handed back reads it with its next letter. SETTLING tells a Lead in passing that bringing its lane into a parallel task's copy at hand-back stopped on conflicts, which that task's Peer settles before it hands back. HUMAN WROTE tells whoever supervises what the Human
