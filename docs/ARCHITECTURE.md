...
| `close_lane` | Supervisor | Waits for every merge the project has queued. With `land`, it lands in one order: when the base has moved on it first merges the base into the lane in the lane's own copy, then runs the lane gate on that, then moves the base up to the lane by fast-forward. Anything that stops a landing — a seat mid-turn in the lane's copy, a conflict with the base, a red gate, a base that cannot be moved — refuses the call and leaves the lane open. It then cuts leftover tasks, archives their seats and the Lead, and puts the copy away |
